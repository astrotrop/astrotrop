...
At the other end of the scale, \cite{eml} is a detailed set of
...
The design issues identified with the \cite{ivoa} software would prevent
...
capabilities of some of the current \cite{gis} based systems which are
designed to handle geographical and ecological data.
...
The \citetitle*{fgdc-geo} (\cite{fgdc}) \citetitle*{fgdc-geo} is designed to :
...
\begin{itemize}
    \item Scientists and engineers from the community members.
    \item Specialist working groups looking at specific areas.
    \item Development based on open source model.
    \item Standards evolve as a result of public discussion.
...
\cite{gis} based systems such as \cite{metacat} and \cite{ckan}.

Both \cite{metacat} and \cite{ckan} already provide support for the \cite{gis}
and \cite{eml} metadata models.

In addition, both \cite{metacat} and \cite{ckan} are involved in large
scale cross disciplinary data management projects.
...
A number of the \cite{trop} science cases are based on remote sensing data
from sources such as \cite{landsat} and on processed data products from
groups such as \cite{nasa-jpl-carbon} and \cite{whrc}.

Ideally, in the long term, the best way to support these \cite{trop}
science cases would be to encourage these external data providers to join
the \cite{trop} community and adopt the same metadata and service standards
for data discovery and data access.
 
Basing the \cite{trop} system on software that is already being used by
projects that handle earth observational and geospatial data and use the
\cite{gis} and \cite{eml} metadata models is likely to make it easier
encourage data providers such as \cite{nasa-jpl-carbon} and \cite{whrc}
to participate.

\subsection{Project structure}

However, that said, we strongly recomend adopting the political structure
and development processes from the \cite{ivoa} and \cite{astro} projects.
...
Over 10 years after it was established the \cite{ivoa} is still an active
and thriving community of scientists and engineers working on developing
the next generation of servces and specifications needed to support the
latest research techniques and for handling and processing the huge data
volumes the next generation of telescopes and satelite missions will generate.

Much of that success is due to the structures and processes put inplace
early in the \cite{astro} and \cite{ivoa} projects.
